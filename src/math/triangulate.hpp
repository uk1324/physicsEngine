...
struct SimplePolygonTriangulator {
	...
	Actually this self intersection was probably caused by the intersectsShape function checking endpoints with epsilons. Disabling it fixes this example, but there are probably cases when it would make it impossible to find a hole cut.
